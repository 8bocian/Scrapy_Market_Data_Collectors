...
class CompanierSpider(CrawlSpider):
    data_path = "data/data.jsonlines"
    data_path_parts = [part for part in data_path.split("/")]

    if not path.isfile(data_path):
        for data_path_part in data_path_parts[:-1]:
            system(f"mkdir {data_path_part}")
        system(f"type nul > {data_path_parts[-1]}")
    ...
    def display_info(self):
        ...
        while 1:
            ...
            speed = (self.data_counter / (time() - self.start_time + 0.0001))
    ...
    def start_requests(self):
        ...
        try:
            with open(self.data_path, encoding='utf-8') as file:
                for line in file:
                    self.prev_tickers.append(list(loads(line).keys())[0])
        ...
        yield Request(url=self.start_urls[0], callback=self.get_page)
